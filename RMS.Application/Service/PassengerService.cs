    ...
    ) : IPassenger
{
    ...
    public async Task<Guid> AddAsync(CreatePassengerCommand argo, CancellationToken token = default)
    {
        ...
        if (accidentId == Guid.Empty)
            throw new KeyNotFoundException("خودروی ارسالی یافت نشد");
        if (await _repNativeCode.DuplicateNationalCodeAsync(argo.NationalCode, accidentId, token))
            throw new InvalidOperationException("کد ملی یک بار در این تصادف ثبت شده");
        if (await _repPass.OneDriverValidAsync(argo.FkCar, argo.IsDriver, token))
            throw new InvalidOperationException("برای این خودرو قبلا  راننده ثبت شده است ");
    ...
    public async Task<bool> DeleteAsync(DeletePassengerCommand argo, CancellationToken token = default)
    {
        if (argo.Id == Guid.Empty)
            throw new ValidationException("شناسهٔ سرنشین الزامی است.");
    ...
    public Task<PagedResponse<PassengerListItemResponse>> GetAllAsync(PassengerQueries argo, CancellationToken token = default)
    {
        if (!argo.TryValidate(out var validationErrors))
            throw new ValidationException(validationErrors.JsonErrors());
        if (argo.DamageType.HasValue && !Enum.IsDefined(argo.DamageType.Value))
            throw new ValidationException("نوع آسیب نامعتبر است.");
    ...
    public async Task<bool> UpdateAsync(UpdatePassengerCommand argo, CancellationToken token = default)
    {
        ...
        if (idcar == Guid.Empty)
            throw new KeyNotFoundException("کد  ارسالی در سرنشین یافت نشد");
        if (await _repPass.OneDriverValidEditAsync(argo.Id, idcar, argo.IsDriver, token))
            throw new InvalidOperationException("برای این خودرو قبلا  راننده ثبت شده است ");
        var idacc = await _repCar.GetParentAsync(idcar, token);
        if (idacc == Guid.Empty)
            throw new KeyNotFoundException("کد خودرو در حادثه یافت نشد");


        argo.InjuryPercentage = argo.TypePersonDamage == DamageTypePersonEnum.Fatal ? (byte)100 : argo.InjuryPercentage;
        argo.NationalCode = argo.NationalCode.NormalizeUnicode();
        if (await _repNativeCode.DuplicateNationalCodeEditAsync(argo.NationalCode, idacc, argo.Id, false, token))
            throw new InvalidOperationException("کد ملی یک بار در این تصادف ثبت شده");
