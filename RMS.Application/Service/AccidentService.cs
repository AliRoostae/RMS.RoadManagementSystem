    ...
    ) : IAccident
{
    ...
    public async Task<Guid> AddAsync(CreateAccidentCommand argo, CancellationToken token = default)
    {
        ...
        if (!await _repRoad.IsValidCoordinate(argo.Latitude, argo.Longitude, argo.FkIdRoad, token))
            throw new InvalidOperationException("لوکشین ارسالی برای این جاده نیست ");
    ...
    public async Task<bool> DeleteAsync(DeleteAccidentCommand argo, CancellationToken token = default)
    {
        ...
        if (await _repCar.AnyCarInAccidenAsync(argo.Id, token))
            throw new InvalidOperationException("برای این رویداد خودرو ثبت شده و امکان حذف آن وجود ندارد.");
        if (await _repPeople.AnyPeopleInAccidenAsync(argo.Id, token))
            throw new InvalidOperationException("برای این رویداد عابر ثبت شده و امکان حذف آن وجود ندارد.");
    ...
    public async Task<PagedResponse<AccidentListItemResponse>> GetAllAsync(AccidentQuery argo, CancellationToken token = default)
    {
        if (!argo.TryValidate(out var validationErrors))
            throw new ValidationException(validationErrors.JsonErrors());
        if (argo.EndTime <= argo.StartTime)
            throw new ValidationException("پایان بازه باید بعد از شروع بازه باشد.");
        if (argo.AccidentType.HasValue && !Enum.IsDefined(argo.AccidentType.Value))
            throw new ValidationException("نوع حادثه نامعتبر است.");
        if (argo.Weather.HasValue && !Enum.IsDefined(argo.Weather.Value))
            throw new ValidationException("وضعیت آب‌وهوا نامعتبر است.");
        if (argo.Cause.HasValue && !Enum.IsDefined(argo.Cause.Value))
            throw new ValidationException("علت حادثه نامعتبر است.");
        if (!Enum.IsDefined(argo.OrderBy))
            throw new ValidationException("ترتیب نمایش حوادث نامعتبر است.");
