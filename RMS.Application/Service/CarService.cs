    ...
    ) : ICar
{
    ...
    public async Task<Guid> AddAsync(CreateCarCommand argo, CancellationToken token = default)
    {
        ...
        if (!await _repAcc.AnyAsync(argo.FkAccident, token))
            throw new KeyNotFoundException(" کد ارسالی تصادفات موجود نیست ");
        if (await _repCar.DuplicatePlateAsync(argo.PlateNumber, argo.FkAccident, token))
            throw new InvalidOperationException(" شماره پلاک تکراری است ");

        var insert = argo.Adapt<CarEntities>();
        insert.Id = Guid.NewGuid();

        if (await _repCar.AddAsync(insert, token))
            return insert.Id;
        else
            return Guid.Empty;
    }
    ...
    public async Task<bool> DeleteAsync(DeleteCarCommand command, CancellationToken token = default)
    {
        if (command.Id == Guid.Empty)
            throw new ValidationException("شناسهٔ خودرو الزامی است.");
        if (await _repPass.AnySubsetPassengerAsync(command.Id, token))
            throw new InvalidOperationException("برای خودرو سرنشین ثبت شده و امکان حذف نیست");
    ...
    public Task<PagedResponse<CarListItemResponse>> GetAllAsync(CarQueries argo, CancellationToken token = default)
    {
        if (!argo.TryValidate(out var validationErrors))
            throw new ValidationException(validationErrors.JsonErrors());
        if (argo.CarClass.HasValue && !Enum.IsDefined(argo.CarClass.Value))
            throw new ValidationException("رده خودرو نامعتبر است.");
    ...
    public async Task<bool> UpdateAsync(UpdateCarCommand argo, CancellationToken token = default)
    {
        ...
        if (find == null)
            throw new KeyNotFoundException(" کد ارسالی خودرو  موجود نیست ");
        argo.PlateNumber = argo.PlateNumber.NormalizeUnicode();
        argo.DriverLicNumber = argo.DriverLicNumber.NormalizeUnicode();
        argo.DriverPhone = argo.DriverPhone.NormalizeUnicode();
        if (await _repCar.DuplicatePlateAsync(argo.PlateNumber, find.FkAccident, argo.Id, token))
            throw new InvalidOperationException(" شماره پلاک تکراری است ");
