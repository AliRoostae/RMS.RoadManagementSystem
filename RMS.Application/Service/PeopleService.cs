    ...
    ) : IPeople
{
    ...
    public async Task<Guid> AddAsync(CreatePeopleCommand argo, CancellationToken token = default)
    {
        ...
        if (!await _repAcc.AnyAsync(argo.FkAccident, token))
            throw new KeyNotFoundException("کد تصادف ارسالی در تصادف یافت نشد");

        argo.NationalCode = argo.NationalCode.NormalizeUnicode();
        if (await _repNativeCode.DuplicateNationalCodeAsync(argo.NationalCode, argo.FkAccident, token))
    ...
    public async Task<bool> DeleteAsync(DeletePeopleCommand argo, CancellationToken token = default)
    {
        if (argo.Id == Guid.Empty)
            throw new ValidationException("شناسهٔ شخص الزامی است.");
    ...
    public async Task<bool> UpdateAsync(UpdatePeopleCommand argo, CancellationToken token = default)
    {
        ...
        if (idAccident == Guid.Empty)
            throw new KeyNotFoundException("کد  ارسالی در عابر یافت نشد");
        argo.InjuryPercentage = argo.TypePersonDamage == DamageTypePersonEnum.Fatal ? (byte)100 : argo.InjuryPercentage;
        argo.NationalCode = argo.NationalCode.NormalizeUnicode();
        if (await _repNativeCode.DuplicateNationalCodeEditAsync(argo.NationalCode, idAccident, argo.Id, true, token))
            throw new InvalidOperationException("کد ملی یک بار در این تصادف ثبت شده");
